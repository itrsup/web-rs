    ...
    <section id="contact" class="contact">
      <div class="container">
        <div class="row mt-5 justify-content-center" data-aos="fade-up">
          <div class="col-lg-10">
            <H4>Mapping fasilitas</H4>

            <form action="layanan_rawat_inap_act.php" method="post">
              <p>Fasilitas</p>
              <div class="mb-2">
                <input type="hidden" name="layanan_id" value="<?php echo $_GET['id'] ?>">
                <select class="form-control js-example-basic-multiple" name="fasilitas_id[]" multiple="multiple">
                  <?php
                  $query = mysqli_query($conn, "SELECT * FROM fasilitas");
                  while ($row = mysqli_fetch_array($query)) {
                  ?>
                    <option value="<?php echo $row['fasilitas_id'] ?>"><?php echo $row['nama_fasilitas'] ?></option>
                  <?php } ?>
                </select>
              </div>
